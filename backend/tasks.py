...
class CreateTaskBody(BaseModel):
    ...
    # User's Claude API key. Sent over HTTPS, held only in agent_runner's
    # coroutine memory, never persisted, never logged.
    anthropic_api_key: SecretStr = Field(min_length=10)
...
async def create_task(
    ...
    # Dispatch the runner. From here, the key lives only inside the runner
    # coroutine's local frame.
    api_key = body.anthropic_api_key.get_secret_value()
